<?php
  ...
  if(!isset($_SESSION['otp_cos_code']))
  {
    ...
    $query = "insert into home_banner(title, image) 
    values('$title', '$image')";
    ...
    if($result)
    {
        $success = 'Home banner has been added successfully';
        include ('add_home_banner.php');
        exit;
        
    }
    else{
        $error = 'Sorry, home banner cannot be added at this time, make sure all information are entered correctly';
        include ('add_home_banner.php');
        exit;
    }
